...
from mcp_project_context_server.helpers.adr import replace_section
...
async def handle(arguments: dict) -> list[types.TextContent]:
    """Handle the ``edit_project`` tool call.

    :param arguments: (dict) Tool input dict. Requires keys ``"project_path"``,
        ``"section"`` (the exact ``##`` heading text, without the ``##``
        marker), and ``"content"`` (the section's new body, excluding the
        heading line). Optional ``"auto_reindex"`` (bool, default False).
    ...
    found, missing = await resolve_requested_files(_project_path, ["project.md"])
    if "project.md" in missing:
        return [
            types.TextContent(
                type="text",
                text=f"Could not read project.md near {arguments['project_path']}",
            )
        ]

    current_content = found["project.md"]
    updated = replace_section(current_content, section_name, new_content)
    if updated is None:
        available = [s.name for s in split_sections(current_content) if s.name]
        available_text = ", ".join(available) if available else "none"
        return [
            types.TextContent(
                type="text",
                text=f"No section named '{section_name}' found in project.md. Available sections: {available_text}.",
            )
        ]

    provider = get_repository_provider()
    resolved_path, is_remote = resolve_project_path(_project_path, provider.provider_name)
    commit_message = f"Update '{section_name}' section of project.md"

    if is_remote:
        message = await write_context_file(provider, resolved_path, "project.md", updated, commit_message)
    else:
        ...
        (context_dir / "project.md").write_text(updated, encoding="utf-8")
        message = f"Updated section '{section_name}' in project.md."
