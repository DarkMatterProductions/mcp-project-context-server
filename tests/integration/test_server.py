...
class TestServerRegistration(MCPIntegrationBase):
    ...
    async def test_server_initialises_successfully(self, mcp_session):
        assert mcp_session is not None

    async def test_all_tools_are_registered(self, mcp_session):
        result = await mcp_session.list_tools()
        registered = {t.name for t in result.tools}
        assert _EXPECTED_TOOLS == registered

    async def test_exactly_five_tools_registered(self, mcp_session):
        result = await mcp_session.list_tools()
        assert len(result.tools) == 5

    @pytest.mark.parametrize("tool_name", sorted(_EXPECTED_TOOLS))
    async def test_each_tool_has_input_schema(self, mcp_session, tool_name):
        result = await mcp_session.list_tools()
    ...
    async def test_load_project_context_requires_project_path(self, mcp_session):
        result = await mcp_session.list_tools()
        tool = next(t for t in result.tools if t.name == "load_project_context")
        assert "project_path" in tool.inputSchema.get("required", [])

    async def test_search_project_context_requires_project_path_and_query(self, mcp_session):
        result = await mcp_session.list_tools()
    ...
    async def test_save_session_summary_requires_project_path_and_summary(self, mcp_session):
        result = await mcp_session.list_tools()
    ...
    async def test_index_project_context_requires_project_path(self, mcp_session):
        result = await mcp_session.list_tools()
        tool = next(t for t in result.tools if t.name == "index_project_context")
        assert "project_path" in tool.inputSchema.get("required", [])

    async def test_list_repositories_has_no_required_fields(self, mcp_session):
        result = await mcp_session.list_tools()
        tool = next(t for t in result.tools if t.name == "list_repositories")
        required = tool.inputSchema.get("required", [])
        assert required == []

    async def test_unknown_tool_returns_error_text(self, mcp_session):
